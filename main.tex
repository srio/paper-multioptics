...
% \title{A fast and light tool-set for partially-coherent simulations of 4$^{\text{th}}$ generation storage ring}    #RC211202: suggestion of new name
...
The use of pure ray-tracing methods based in geometrical optics is often not adequate when analyzing optical systems dealing with (partially- or fully-) coherent beams as conventional ray-tracing does not reproduce diffraction effects. However, ray-tracing-based methods are much simpler and fast than wave-optics-based methods and more often than not, offer useful insight when studying and designing a synchrotron beamline \cite{hierarchical}. By incorporating concepts of coherent diffraction, the effects due to beam clipping by either physical acceptance of an optical element or slits; and optical errors can be added to ray-tracing \cite{Semichaevsky2001}. Building on that hybrid concept, an extension to the ray-tracing code SHADOW \cite{codeSHADOW} has been proposed by \citeasnoun{codeHYBRID}, which is fully integrated in the ShadowOUI \cite{codeSHADOWOUI} add-on in OASYS. Since its release, this hybrid ray-tracing implementation has been successfully used as an efficient and fast tool to design beamlines also including coherence effects \cite{Shi2017,Luca2020}. % The hybrid ray-tracing can reproduce the effect of degradation of the focus shift and focal shift when changing the NA. 
...
\todo{may be is interesting to explain here how to obtain 2D data by outer product of H and V profiles, something that is used later on}


\section{Propagation of wavefronts along the beamline}

\subsection{Wavefront propagation in free-space (Drift spaces)}
...
\subsection{Wavefront cropping (apertures)}
...
\subsection{Wavefront focusing (lenses)}
...
\todo{a possible new section describing the coherent parameters (CF, coherent length, etc) and the way to obtan them via rediagonalization...}
